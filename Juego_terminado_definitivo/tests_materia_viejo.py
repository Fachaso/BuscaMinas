...
from buscaminas_viejo import (crear_juego, descubrir_celda, marcar_celda, obtener_estado_tablero_visible,
...
class TestEstadoValido(unittest.TestCase):
    ...
    def test_estado_valido_invalido(self):
        ...
        """estado:EstadoJuego = {
            ...
            "minas": 1,
            "tablero": [[-1, 2], [2, -1]],
            "tablero_visible": [[BOMBA, BANDERA], ["2", "-1"]],
            "juego_terminado": False
        }
        self.assertFalse(estado_valido(estado))"""
...
class TestCargarEstado(unittest.TestCase):
    ...
    def test_tablero_fuera_de_rango(self):
        ...
        self.escribir_archivo("tablero.txt", [""])
        self.escribir_archivo("tablero_visible.txt", ["*,1", "1,?"])
        estado: EstadoJuego = {}
        resultado:bool = cargar_estado(estado, ".")
        self.assertFalse(resultado)
        ...
        self.escribir_archivo("tablero.txt",["-1,\n","1,\n"])
        self.escribir_archivo("tablero_visible.txt", ["*,1", "1,?"])
        estado: EstadoJuego = {}
        resultado:bool = cargar_estado(estado, ".")
        self.assertFalse(resultado)
    ...
    def test_tablero_visible_caracter_invalido(self):
        ...
        self.escribir_archivo("tablero.txt", ["-1,1", "1,1"])
        self.escribir_archivo("tablero_visible.txt", ["*,1", "?,?", "1,1"])
        estado: EstadoJuego = {}
        resultado:bool = cargar_estado(estado, ".")
        self.assertFalse(resultado)
        borrar_archivos()
...
class TestColocar_extrass(unittest.TestCase):

    
    def test_estado_valido_numero_visible_incorrecto(self):
        estado = crear_juego(2, 2, 1)
        estado['tablero'][0][0] = 3
        estado['tablero_visible'][0][0] = "4"  # Incorrecto
        self.assertFalse(estado_valido(estado))
    ...
    def test_cargar_estado_falso_sin_minas(self):
        self.escribir_archivo("tablero.txt", ["0,0", "0,0"])
        self.escribir_archivo("tablero_visible.txt", ["?,?", "?,?"])
        estado = {}
